...
internal class AcpSession(
    ...
    @Volatile private var imageSupported: Boolean? = null
    @Volatile private var imageListener: (Boolean?) -> Unit = {}

    fun observeImageSupport(listener: (Boolean?) -> Unit) {
        synchronized(lock) { imageListener = listener; listener(imageSupported) }
    }

    private fun publishImageSupport(value: Boolean?) {
        synchronized(lock) {
    ...
    fun send(prompt: String, turn: PreparedAgentTurn, commandText: String? = null, commandName: String? = null,
        ...
        try {
            ...
            if (!turn.run.isActive) return
            if (image != null && imageSupported != true) {
                throw AcpLocalRejection("このACP接続では画像対応を確認できません。対応する接続で再送してください。")
            }
            val promptParams = sessionParams().apply {
                add("prompt", JsonArray().apply {
                    ...
                    if ((commandText == null && image == null) || prompt.isNotEmpty()) add(jsonObject("type" to "text", "text" to prompt))
                    if (image != null) add(jsonObject("type" to "image", "mimeType" to "image/png",
                        "data" to java.util.Base64.getEncoder().encodeToString(image.bytes())))
                })
            }
    ...
    private fun connect(root: String, executable: String, isActive: () -> Boolean): AcpJsonRpc = synchronized(connectionLock) {
        ...
        connection.request("initialize", initialize) {
            ...
            val capability = body["agentCapabilities"]?.takeIf { it.isJsonObject }?.asJsonObject
                ?.get("promptCapabilities")?.takeIf { it.isJsonObject }?.asJsonObject?.get("image")
            publishImageSupport(capability?.takeIf { it.isJsonPrimitive && it.asJsonPrimitive.isBoolean }?.asBoolean == true)
