...
現在のprint経路は、手動指定に実行権限があればそれを選び、なければ固定候補、最後に実行時PATHのagentへ戻る。一方ACP経路は非空の手動指定をそのまま渡す。設定診断は起動方式を選ばないため、不正な手動指定は未解決とし、自動へ戻ることや成功を一律に約束しない。絶対パスの実行可能ファイルでも「ファイル確認のみ。CLI起動は未確認」であり、正しいCLI版・認証成功を保証しない。相対パスは解決先未確認、候補なしはPATH検索が未確認とする。候補がdirectory等なら実行可能ファイル確認失敗として示す。探索規則やservice実行を変更しない。
