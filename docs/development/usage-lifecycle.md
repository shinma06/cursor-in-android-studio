...
StateはEDT所有。beginは旧値を消して新ticketを発行、finishは成功/停止/失敗を一度だけ確定してticketを失効させる。終端後のusageや古い終端は保持した値・次の応答を変更できない。resetは値/送信時modelを消して未開始へ戻し、旧ticketを無効にする。Factoryは既存run/token・dispose・Stopの検査をUI配送直前に行い、その内側でticketを照合する。
