...
2026-09-06の移行時は実行中タスクのため、旧パスに一時的な互換symlinkを残した。
その後、ユーザーの明示指示により互換リンクは撤去済みであり、現在は上記の新実体パスを使用する。
一般に互換リンクを撤去する際は、既存タスク・IDE・Codexの登録先・自動処理・引継ぎ状態が
新パスへ移り、旧パスを使う処理がないことを事前に確認する。
