...
| ID | Branch | PR | 観点 | 手順 | 期待結果 | Status | Verified by | Date |
|----|--------|-----|------|------|----------|--------|-------------|------|
| MV-042 | `main`（run SHA参照） | - / #27 #20 | 設定の全文・上方向popup・Agent背景 | チャット設定の値と項目名を確認。モデル/モードを開き、同じボタン/外側フォーカス/Escapeで閉じる。検索・Autoの高さ変更を確認 | 値が余白を使って全文表示。指定の日本語項目名。ボタンを隠さず上に展開、閉じる操作と高さ変更で位置を維持。Agentはグレー背景 | pending | | |
