...
| ID | Branch | PR | 観点 | 手順 | 期待結果 | Status | Verified by | Date |
|----|--------|-----|------|------|----------|--------|-------------|------|
| MV-040 | `main`（run SHA参照） | - / #20 #21 | 日本語の詳細設定・通常画面の簡潔化 | 入力欄→「…」→各選択肢を開きEscapeで取消、プラグイン設定/MCPを閲覧 | 入力下の文面なし。3設定の現在値・選択肢・注意・操作が日本語。未確定で設定変更なし。入力/Agent/モデル/placeholderの外観を維持。即時編集説明は詳細設定とSettings内に表示 | pending | | |
