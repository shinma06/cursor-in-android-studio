...
| 知識 / `CLAUDE.md`・`docs/` | 検証可能な根拠 → 方針・現行設計・判断 | 該当設計のwriter、別sessionが根拠と適用範囲をレビュー | [Mission](../project-mission.md)が目的、[ACP First](cursor-integration.md)が方針、[現行実装](current-implementation.md)が実装説明。経緯はIssue/PR、版付き実測はresearch/Case。昇格・履歴整理は#228 |
